DROP DATABASE IF EXISTS vk_db_s5;
CREATE DATABASE vk_db_s5;
USE vk_db_s5;
...
SELECT CONCAT(u.firstname,' ', u.lastname) AS  name_frends FROM users u JOIN
(SELECT target_user_id FROM friend_requests WHERE initiator_user_id = 1 AND status = 'approved') AS r
ON u.id = r. target_user_id;

-- (решение задачи с помощью представления “друзья”)
CREATE OR REPLACE VIEW frends AS
SELECT r.initiator_user_id, CONCAT(u.firstname, ' ', u.lastname) AS name_frends FROM users u JOIN
(SELECT target_user_id, initiator_user_id FROM friend_requests WHERE  status = 'approved') AS r
ON u.id = r. target_user_id;
SELECT name_frends FROM frends WHERE initiator_user_id = 1;
-- Создайте представление, в котором будут выводится все сообщения, в которых принимал
-- участие пользователь с id = 1
CREATE OR REPLACE VIEW v1 AS
SELECT body as text_message FROM messages WHERE from_user_id = 1 OR to_user_id = 1;
-- Получите список медиафайлов пользователя с количеством лайков(media m, likes l ,users u)
SELECT * FROM (SELECT m.filename, COUNT(l.id) AS all_likes, CONCAT(u.firstname, ' ', u.lastname) AS name_user  FROM media m
LEFT JOIN likes l ON l.media_id = m.id
JOIN users u  ON u.id = m.user_id
GROUP BY m.id) AS t
WHERE t.all_likes > 0;
-- Получите количество групп у пользователей
SELECT us. user_id, CONCAT(u.firstname, ' ', u.lastname) AS user_name, COUNT(us.user_id) AS count_gr FROM users_communities us
...
SELECT u.firstname, u.lastname, p.hometown, p.gender FROM profiles p 
...
SELECT t.user_name, t.count_from,
DENSE_RANK() OVER(ORDER BY count_from DESC ) AS 'DENSE_RANK' FROM
( SELECT CONCAT(u.firstname, ' ', u.lastname) AS user_name,COUNT(m.from_user_id) AS count_from FROM messages m
...
SELECT t.body, t.created_at, timediff(t.created_dt_next, t.created_at) AS diff_time_mes FROM
(SELECT body, created_at, LEAD(created_at) OVER(ORDER BY created_at) AS created_dt_next FROM messages)as t;
